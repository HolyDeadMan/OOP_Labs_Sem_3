...
void menu()
{
	...
	std::cout << "6) Print c tree iterator" << std::endl;
	std::cout << "7) Sorting tree" << std::endl;
	std::cout << "8) Balancing  tree using a parallel sorting" << std::endl;
...
int main(void)
{
	...
	int n;
	std::cout << "To access the menu, press 9" << std::endl;
	do {
		std::cin >> act;
		switch (act) {
		case 1:
		...
		case 7:
			...
			std::cin >> n;
			for (int i = 0; i < n; ++i) {
				std::cout << "Left to enter " << n - i << " vertices" << std::endl;
				ptr = std::make_shared<Hexagon>(std::cin);
				arr[i] = ptr;
			}
