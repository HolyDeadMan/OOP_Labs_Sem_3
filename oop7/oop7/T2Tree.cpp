...
template <class Q, class O>
void T2Tree<Q, O>::recRemByType(std::shared_ptr<Node>& node, int& type)
{
	if (node->itemsInNode) {
...
template <class Q, class O>
void T2Tree<Q, O>::removeByType(int& type)
{
	recRemByType(root->son, type);
}

template <class Q, class O>
void T2Tree<Q, O>::recInsert(std::shared_ptr<Node>& node, O& item)
{
...
template <class Q, class O>
void T2Tree<Q, O>::recInorder(std::shared_ptr<Node>& node)
{
	if (node->itemsInNode) {
		for (auto i : node->data) {
			i->Print();
...
template <class Q, class O>
void T2Tree<Q, O>::recRemLesser(std::shared_ptr<Node>& node, int32_t& side)
{
	if (node->itemsInNode) {
		for (auto i : node->data) {
			if (i->Side() == side) {
...
template <class Q, class O>
void T2Tree<Q, O>::removeLesser(int32_t& side)
{
	recRemLesser(root->son, side);
}

template <class Q, class O>
void T2Tree<Q, O>::clear(std::shared_ptr<Node>& node, std::shared_ptr<Node>& parent)
{
	if (node) {
		if (!node->itemsInNode) {
			auto orphan = node;
...
template <class Q, class O>
void T2Tree<Q, O>::GG(std::shared_ptr<Node>& node)
{
	if (node->itemsInNode) {
		node->data.gg();
		if (node->sibling) {
...
#include "T2Tree.h"
template class T2Tree<TTree<Figure>, std::shared_ptr<Figure>>;
