...
int main(int argc, char** argv)
{
	...
	while (!std::cin.eof()) {
		...
		if (action == "q" || action == "quit") {
			...
			if (!(std::cin >> size_a  >> key)) {
				std::cout << "Incorrect value." << std::endl;
				continue;
			}
			TreeItem * newNode = new TreeItem(Hexagon(size_a));
			...
			std::cout << "'r' or 'remove s' - remove the hexagon with area s." << std::endl;
			std::cout << "'d' or 'destroy' - delete the tree." << std::endl;
			std::cout << "'p' or 'print' - output the tree." << std::endl;
			std::cout << "'ins' or 'insert s' - insert a hexagon into the tree." << std::endl;
